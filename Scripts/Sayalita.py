...
token, stopUser, notifications_channel, riotAPIKey, gameName, gameTag = FileReader.read_files()
...
async def on_ready() -> None:
  global points
  if os.path.getsize("../TextFiles/points.json") != 0:
...
def getAccountPUUID(gameName: str, tagLine: str) -> str:
  url = f'https://americas.api.riotgames.com/riot/account/v1/accounts/by-riot-id/{gameName}/{tagLine}'
  response = requests.get(url, headers={'X-Riot-Token': riotAPIKey})
  data = response.json()['puuid']

  return data

def getMatchesFromPUUID(puuid: str, count: int) -> list[str]:
  url = f'https://americas.api.riotgames.com/lol/match/v5/matches/by-puuid/{puuid}/ids'
  response = requests.get(url, params={'api_key': riotAPIKey, 'count': count})
  data = response.json()
  
  return data

def getMatchFromMatchID(matchid: str) -> dict:
  url = f"https://americas.api.riotgames.com/lol/match/v5/matches/{matchid}/timeline?api_key={riotAPIKey}"
  response = requests.get(url)
  data = response.json()

  return data

def parseMatchData(matchData: dict) -> dict:
  frames = matchData['info']['frames']
  final_frame = len(frames) - 1
  data = frames[final_frame]['participantFrames']
